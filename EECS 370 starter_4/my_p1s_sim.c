...
int main(int argc, char **argv)
{
    ...
    //grab the block size, number of sets and blocks per set
   ...
   // printState(&state);
    ...
    {
        int lineCode = cache_access(state.pc,0,0);
        int temp1 = lineCode;
        ...
        else if (opcode == 0b010||opcode == 0b011||opcode == 0b100)
        {
            ...
            if (opcode == 0b010)
            {
                ...
                state.reg[regB]= cache_access(index,0,0);
    ...
    printState(&state);
    printf("%d num mem accesses",num_mem_accesses);
    // Your code ends here!
