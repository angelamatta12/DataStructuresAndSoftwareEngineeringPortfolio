...
//instance of key_compare
  //key_compare = std::less<Key_type> comp;
   Key_compare comp;
...
//key leads to value 
//....... AHHHAhahah have to use binary 
// search tree. ^^^^^^^^^^
//datum will be the pair 
 ...
 return entries.find({k,Value_type()});
  ...
  Value_type& operator[](const Key_type& k){
   ...
   // return &node ?? 
    ...
    if(iter == end()){
    //idk if that works
    //trying to access second 
    //elem of iter 
     entries.insert({k, Value_type()});
     iter = find(k);
     //am I returning the right thing??
     return iter->second;

  }

//need to use different insert 
  ...
  std::pair<Iterator, bool> insert(const Pair_type &val){
      ...
      //if(find)
      //return iter @ val, false

     //searched to see if given key in map 
      Iterator iter = find(val.first);

       if(iter != end()){
         std::pair<Iterator, bool> p(iter,false);
          return p;
       }
     ...
     //return iter @ val, true 
       ...
       //insert pair 
       iter = entries.insert(val); 
       std::pair<Iterator, bool> p(iter,true);
       return p;
