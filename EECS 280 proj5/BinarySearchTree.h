...
//must check
  ...
  // NOTE:    This function must run in constant time. O(1)
...
//got from lecture 
  ...
  static int height_impl(const Node *node) {
    //at leaf or empty boi 
   if(!node){
     return 0;

   }
   //dont know if right max 
...
///CHECK 
  ...
  //at empty boi or leaf's nullptr
 if(!node){
    ...
    //definitely 
    // use node-> left and right 
    //must nav to leaf
    //work all the way down to leaf 
    //works backwards deleting from there 
    //ensure to set the rooted at node 
    //to nullptr 
  Node *n = new Node();
...
//empty boi, 
   ...
   //have a right not a left 
  //get to the bottom first 
   destroy_nodes_impl(node->left);
   destroy_nodes_impl(node->right);

   //deletes the root that
   //was passed in 
   delete node;
   node = nullptr;
...
// not returning correctly when 
//obj isnt found 
 //empty boi//reach a leaf's nullptr w/o finding 
 // the val 
...
//check if element node is less than query
//yes check right of tree 
...
return find_impl(node->left, query, less);
  ...
  // aka the functor == false ??
  // op of < means >= 
  // need to ensure the vals are not > 
  //that is what is up with the whole a !< b
  // b !< a so only can be ==
...
//CHECK LOGIC
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) {
   ...
   //possible cases this would activate
   // empty boi just add it in
      //leaf's nullptr too
  //concerned about the whole
  //leaf's nullptr bit. 
  // does it return right for that instance .
  if(!node){

   Node *p = new Node(item, nullptr, nullptr);
   return p;
...
//switched it for the recur 
//call in else if 
//ptr made outside. 
//now returns root correctly 
//rootonly elements 
...
//each time we need to check the 
//L and R of the current node against 
//val before returning

//either of those nodes could also fit bill 

// empty boi, or no elements > val 
if(!node){
...
//L to get min tree item 

//other base cases
//Have L -> root node is answer
//Have L -> L is answer 
//dont have L -> root node is answer 
else {

Node *lCheck = min_greater_than_impl(node-> left, val, less);
...
//just putting this here to make it
//compile need to check this function 
...
}; // END of BinarySearchTree class d
